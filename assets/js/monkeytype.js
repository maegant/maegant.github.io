...
//   function saveScore(player, wpm, acc) {
...
//         // var thisUser = player;
//         // if (run.user) { player = run.user.firstName; };
...
// };
  ...
  function addToLeaderboard(wpm, acc){


    var rank = findRank(wpm);
    if (rank != 0){
        var playername = prompt("Enter your name in the leaderboard!");
        if (playername !== null) {
            addPlayer(rank,playername,wpm, acc);
        }
    }
  }

  function getWPM() {
    var wpm = window.prompt("Enter your wpm");
    if (wpm === null){
        // window.prompt("Entered Cancel")
    }
    //else {window.prompt(wpm)}
    return wpm;
  }

  function getPlayerName() {
    do {
      playername = window.prompt("Enter your name in the leaderboard! Or press Cancel to decline entering your score.");
      return playername;
    }
    while (!playername);
  }

  // Find rank based on wpm
  function findRank(wpm){
    var table = document.querySelector('#leaderboard');
    var rows = table.querySelectorAll("tr");
    for(i=1;i<rows.length;i++){
        var tableData = rows[i].querySelectorAll("td");
        if (tableData[2].textContent < wpm){
            return i;
        }
    }
    return i;
  ...
  function addPlayer(rank, name, wmp, acc){
  ...
  function updateRankings(table, rank) {
    var rows = table.querySelectorAll("tr");
    for (i=rank+1;i<rows.length;i++){
        var tableData = rows[i].querySelectorAll("td");
        tableData[0].textContent = i;
    }
  }
  ...
  function timesUp() {
    hideCaret();
    testActive = false;
    var stats = showResult();
    addToLeaderboard(stats.wpm, Math.round(stats.acc));
  }
